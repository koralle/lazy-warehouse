...
var RoleTableColumns = struct {
	...
	ID:        "roles.id",
	Name:      "roles.name",
	CreatedAt: "roles._created_at",
	UpdatedAt: "roles._updated_at",
}
...
var RoleWhere = struct {
	...
	ID:        whereHelperstring{field: "\"lazy_warehouse\".\"roles\".\"id\""},
	Name:      whereHelperstring{field: "\"lazy_warehouse\".\"roles\".\"name\""},
	CreatedAt: whereHelpertime_Time{field: "\"lazy_warehouse\".\"roles\".\"_created_at\""},
	UpdatedAt: whereHelpertime_Time{field: "\"lazy_warehouse\".\"roles\".\"_updated_at\""},
}
...
var (
	roleAllColumns            = []string{"id", "name", "_created_at", "_updated_at"}
	roleColumnsWithoutDefault = []string{"name"}
	roleColumnsWithDefault    = []string{"id", "_created_at", "_updated_at"}
...
func Roles(mods ...qm.QueryMod) roleQuery {
	mods = append(mods, qm.From("\"lazy_warehouse\".\"roles\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"lazy_warehouse\".\"roles\".*"})
	}
...
func FindRole(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*Role, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"lazy_warehouse\".\"roles\" where \"id\"=$1", sel,
	)
...
func (o *Role) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"lazy_warehouse\".\"roles\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"lazy_warehouse\".\"roles\" %sDEFAULT VALUES%s"
		}
...
func (o *Role) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"lazy_warehouse\".\"roles\" SET %s WHERE %s",
...
func (o RoleSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"lazy_warehouse\".\"roles\" SET %s WHERE %s",
...
func (o *Role) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"lazy_warehouse\".\"roles\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *Role) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"lazy_warehouse\".\"roles\" WHERE \"id\"=$1"
...
func (o RoleSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"lazy_warehouse\".\"roles\" WHERE " +
...
func (o *RoleSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"lazy_warehouse\".\"roles\".* FROM \"lazy_warehouse\".\"roles\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, rolePrimaryKeyColumns, len(*o))
...
func RoleExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"lazy_warehouse\".\"roles\" where \"id\"=$1 limit 1)"
